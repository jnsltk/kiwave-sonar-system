...
int KiwiSonic::ping() {
  ...
  //is the longest the ultrasonic awaits the trasmitted waves. It is put 100 times longer
  //than the time allocated for transmitting waves which is _maxdelay
}
...
his was found out using experimentation
...
distance = speed * time
However the formulaw below is more complicated as it considers temprature as well.
...
at 0 degrees celsius. The two mentioned formulea above are cobined and they give off 
the following formula used in the method below. The units of the different paramters
should be compatible. The disnatce is wanted in cm, hence the speed should be multiplied by
100 to be cm/s. However, ping() gives time in microseconds. Hence the speed needs to be
divided by 1000000. That is where 10000 come from as (speed/1000000) * 1000 is speed/10000.
